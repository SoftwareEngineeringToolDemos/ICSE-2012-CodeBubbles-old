...
public BuenoProjectCreator()
{
   ...
   panel_map = new HashMap<BuenoProjectMaker,JPanel>();
...
{
   if (!BumpClient.getBump().getOptionBool("bedrock.useAndroid")) return;
   boolean androidneeded = false;

   for (File f : project_props.getSources()) {
      androidneeded |= checkAndroidFile(f);
      if (androidneeded && project_props.get(PROJ_PROP_ANDROID_PKG) != null) break;
    }
   if (!androidneeded) return;
   project_props.put(PROJ_PROP_ANDROID,"TRUE");
}


private boolean checkAndroidFile(File f)
{
   boolean fg = false;
   if (f.isDirectory()) {
      ...
      if (subs != null) {
	 for (File f1 : subs) {
	    fg |= checkAndroidFile(f1);
	    if (fg && project_props.get(PROJ_PROP_ANDROID_PKG) != null) break;
	  }
       }
    }
