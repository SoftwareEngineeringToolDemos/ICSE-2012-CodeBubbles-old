			 ...
			 IDocument doc)
{
   ...
   if (module_name != null && module_name.endsWith("__init__")) {
      ...
      builtinCompletions.add(new SourceToken(new Name("__path__",Name.Load,false),
...
@Override public Object visitAttribute(Attribute node) throws Exception
{
   ...
   if (node.ctx == Attribute.Store || node.ctx == Attribute.Param
	  || node.ctx == Attribute.KwOnlyParam || node.ctx == Attribute.AugStore) {
...
@Override public Object visitName(Name node) throws Exception
{
   ...
   if (node.ctx == Name.Store || node.ctx == Name.Param || node.ctx == Name.KwOnlyParam
	  || (node.ctx == Name.AugStore && found)) { // if it was undefined on augstore,
