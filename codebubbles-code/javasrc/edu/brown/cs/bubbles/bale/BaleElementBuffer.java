...
{
   if (be.isElided()) {
      if (elide_regions == null) elide_regions = new HashMap<BaleRegion,BaleElement>();
...
private static class ProblemData {
   ...
   boolean containsPosition(int pos) {
      if (problem_region.getStart() <= pos &&
	     problem_region.getEnd() > pos) return true;
      return false;
    }
...
{
   if (bp.getErrorType() == BumpErrorType.ERROR) return BALE_PROPERTIES.getColor(BALE_ERROR_COLOR);
   else if (bp.getErrorType() == BumpErrorType.WARNING) return BALE_PROPERTIES.getColor(BALE_WARNING_COLOR);
