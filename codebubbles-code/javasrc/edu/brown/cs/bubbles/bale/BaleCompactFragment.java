...
private final static String HIGHLIGHT_START = "<span style='background:#ffff88'>";
...
private void setupLabel()
{
   ...
   rounded_corner_label = new SwingRoundedCornerLabel(BUSS_STACK_TOP_COLOR, BUSS_STACK_BOTTOM_COLOR,
							 aryseparateloc, aryseparateheight, 1);
...
private static class LineData {
   ...
   LineData(int lno) {
      line_number = lno;
      line_highlights = new ArrayList<BaleSimpleRegion>();
    }
...
private class ContentData {
   ...
   private void generateHTML(int startIndex, int endIndex) {
	 ...
	 if ((index & 1) == 1) {
	    middleHtmlBuf.append(HIGHLIGHT_START);
	 ...
	 if ((startAryIndex % 2) == 1) {
	    htmlBuf.append(HIGHLIGHT_START);
