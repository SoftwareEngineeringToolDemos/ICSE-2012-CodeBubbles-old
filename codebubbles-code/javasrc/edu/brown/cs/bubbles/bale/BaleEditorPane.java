...
protected BaleEditorPane()
{
   ...
   active_regions = new ArrayList<ActiveRegion>();
...
@Override public Color getCaretColor()				{ return BALE_CARET_COLOR; }
...
{
   int loc = viewToModel(evt.getPoint());
   BaleDocument bd = getBaleDocument();
...
@Override protected void paintComponent(Graphics g0)
{
   ...
   if (BALE_PROPERTIES.getBoolean(VISUALIZATION_GRADIENT_ENABLE)) {
      ...
      Color tc = BALE_PROPERTIES.getColor(BALE_EDITOR_TOP_COLOR);
      Color bc = BALE_PROPERTIES.getColor(BALE_EDITOR_BOTTOM_COLOR);
...
private static class BaleTextUI extends TextUI {
   ...
   @Override public String getToolTipText(JTextComponent t,Point pt) {
      readLock(t);
      try {
	 return base_ui.getToolTipText(t,pt);
       }
      finally { readUnlock(t); }
    }

   @Override public Rectangle modelToView(JTextComponent t,int pos) throws BadLocationException {
      return modelToView(t,pos,Position.Bias.Forward);
    }

   @Override public Rectangle modelToView(JTextComponent t,int pos,Position.Bias bias)
		throws BadLocationException {
      readLock(t);
      try {
	 return base_ui.modelToView(t,pos,bias);
       }
      finally { readUnlock(t); }
    }

   @Override public int viewToModel(JTextComponent t,Point pt) {
      return viewToModel(t,pt,discard_bias);
    }

   @Override public int viewToModel(JTextComponent t,Point pt,Position.Bias [] bret) {
      readLock(t);
      try {
	 return base_ui.viewToModel(t,pt,bret);
       }
      finally { readUnlock(t); }
    }
