...
private void instTextField(JTextPane j)
{
   ...
   j.setBackground(BALE_PROPERTIES.getColor(BALE_EDITOR_TOP_COLOR));
   j.setOpaque(true);
...
private synchronized void setFragmentName(String name,boolean dirty)
{
   ...
   if (!is_dirty || force_rename) {
	 ...
	 if (last_name != null) {
      ...
      if (size >= 3) {
	 component_list.get(size-3).setColor(BALE_CRUMB_PACKAGE_COLOR);
	 component_list.get(size-2).setColor(BALE_CRUMB_CLASS_COLOR);
	 component_list.getLast().setColor(BALE_CRUMB_METHOD_COLOR);
	 ...
	 if (!s.contains("(") && !s.contains("<")) {
	    component_list.get(size-2).setColor(BALE_CRUMB_PACKAGE_COLOR);
	    component_list.getLast().setColor(BALE_CRUMB_CLASS_COLOR);
	  }
	 else {
	    component_list.get(size-2).setColor(BALE_CRUMB_CLASS_COLOR);
	    component_list.getLast().setColor(BALE_CRUMB_METHOD_COLOR);
	 ...
	 component_list.getLast().setColor(BALE_CRUMB_PACKAGE_COLOR);
       }
      repaint();
    }
