...
private synchronized void handleFound(Collection<BumpCompletion> fnd,boolean calls)
{
   ...
   found_completions = new TreeSet<BumpCompletion>(new CompletionComparator());
	...
	BuenoConstants.BuenoBubbleCreator
{
   ...
   String getCompletionText()		{ return "("; }

   int getStartIndex()			{ return -1; }

   boolean canStartWith(String text) {
      method_name = text;
      if (text.length() == 0 || !isValidId(text)) return false;
      return true;
    }

   Icon getIcon()			{ return BoardImage.getIcon("default_method"); }
