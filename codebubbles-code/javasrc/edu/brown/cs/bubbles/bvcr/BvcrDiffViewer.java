...
private class ChangeHighlighter implements Highlighter.HighlightPainter {
   ...
   @Override public void paint(Graphics g,int off0,int off1,Shape bnds,JTextComponent c) {
      ...
      try {
         ...
         Rectangle r0 = c.modelToView(off0);
         Dimension sz = c.getSize();
         Rectangle pt;
         if (off0 == off1) {
            pt = new Rectangle(0,r0.y+r0.height,sz.width,1);
          }
         else {
            Rectangle r1 = c.modelToView(off1-1);
            pt = new Rectangle(0,r0.y,sz.width,r1.y+r1.height-r0.y);
          }
         g.setColor(color);
         g2.fill(pt);
       }
