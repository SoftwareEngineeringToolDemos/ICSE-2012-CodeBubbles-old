...
class RebaseJavaElider implements RebaseConstants.RebaseElider, RebaseJavaConstants, RebaseConstants
{
...
{
   RebaseJcompSemantics jf = (RebaseJcompSemantics) rsd;
   ...
   if (n instanceof Name) {
      ...
      switch (p.getNodeType()) {
         ...
         case ASTNode.METHOD_DECLARATION :
            ...
            else if (spd == MethodDeclaration.THROWN_EXCEPTIONS_PROPERTY) typ = "TYPE";
            break;
...
private class ElidePass2 extends ASTVisitor {
   ...
   private void outputDeclInfo(Name name) {
      JcompSymbol js = JcompAst.getDefinition(name);
      if (js == null) return;
      ...
      switch (js.getSymbolKind()) {
         ...
         case CONSTRUCTOR :
            ...
            buf.append("(");
            int ct = 0;
            for (JcompType pty : js.getType().getComponents()) {
               if (ct++ > 0) buf.append(",");
...
private static class DefaultPrioritizer extends Prioritizer {
   ...
   double getPriority(double ppar,ASTNode base) {
