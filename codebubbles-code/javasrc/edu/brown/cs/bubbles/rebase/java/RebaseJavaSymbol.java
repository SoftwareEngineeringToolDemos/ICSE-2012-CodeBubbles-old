...
private static class KnownField extends RebaseJavaSymbol {
   ...
   RebaseJavaType getType()		      { return field_type; }
   boolean isKnown()			{ return true; }
   boolean isFieldSymbol()		{ return true; }
   boolean isStatic()			{ return is_static; }
   boolean isFinal()			{ return is_final; }
   RebaseJavaType getClassType()	      { return class_type; }
...
private static class VariableSymbol extends RebaseJavaSymbol {
   ...
   String getName()			{ return ast_node.getName().getIdentifier(); }
   RebaseJavaType getType()		{ return java_type; }
   boolean isUsed()			{ return is_used; }
   void noteUsed()			{ is_used = true; }

   ASTNode getDefinitionNode() {
   ...
   ASTNode getNameNode()		{ return ast_node; }

   boolean isFieldSymbol() {
      for (ASTNode p = ast_node; p != null; p = p.getParent()) {
	 switch (p.getNodeType()) {
	    case ASTNode.FIELD_DECLARATION :
	       return true;
   ...
   String getHandle(RebaseFile rf) {
...
private static class EnumSymbol extends RebaseJavaSymbol {
   ...
   String getName()			{ return ast_node.getName().getIdentifier(); }
   RebaseJavaType getType()		{ return java_type; }
   boolean isEnumSymbol()		{ return true; }
   ASTNode getNameNode()		{ return ast_node; }
   RebaseSymbolKind getSymbolKind()	{ return RebaseSymbolKind.FIELD; }
...
private static class LabelSymbol extends RebaseJavaSymbol {
   ...
   String getName()			{ return ast_node.getLabel().getIdentifier(); }
   RebaseJavaType getType()		{ return null; }
...
private static class MethodSymbol extends RebaseJavaSymbol {
   ...
   RebaseJavaType getType()	      { return RebaseJavaAst.getJavaType(ast_node); }

   ASTNode getDefinitionNode()	{ return ast_node; }
   ASTNode getNameNode()	{ return ast_node; }

   boolean isMethodSymbol()	{ return true; }
   boolean isConstructorSymbol() { return ast_node.isConstructor(); }

   boolean isStatic()		{ return Modifier.isStatic(ast_node.getModifiers()); }
   boolean isPrivate()		{ return Modifier.isPrivate(ast_node.getModifiers()); }
   boolean isFinal()		{ return Modifier.isFinal(ast_node.getModifiers()); }
   boolean isAbstract() 	{ return Modifier.isAbstract(ast_node.getModifiers()); }

   @Override boolean isUsed()	{ return is_used; }
   @Override void noteUsed()	{ is_used = true; }

   RebaseJavaType getClassType() {
...
private static class KnownMethod extends RebaseJavaSymbol {
   ...
   RebaseJavaType getType()		      { return method_type; }
   boolean isKnown()			{ return true; }
   boolean isMethodSymbol()		{ return true; }
   boolean isStatic()			{ return is_static; }
   Iterable<RebaseJavaType> getExceptions()   { return declared_exceptions; }
   RebaseJavaType getClassType()	      { return class_type; }
   boolean isGenericReturn()		{ return is_generic; }
   boolean isConstructorSymbol()	{ return method_name.equals("<init>"); }
...
private static class TypeSymbol extends RebaseJavaSymbol {
   ...
   ASTNode getDefinitionNode()		{ return ast_node; }
   ASTNode getNameNode()		{ return ast_node; }
   boolean isTypeSymbol()		{ return true; }
   ...
   String getFullReportName()		{ return getFullName(); }
