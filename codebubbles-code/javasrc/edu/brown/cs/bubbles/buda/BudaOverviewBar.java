...
@Override protected void paintComponent(Graphics g0)
{
   ...
   if (g != null) {
	 ...
	 if (BUDA_PROPERTIES.getBoolean(OVERVIEW_STYLIZED_VIEW_BOOL)){
	    g.setColor(OVERVIEW_STYLIZED_VIEW_BORDER_COLOR);
	    g.setStroke(new BasicStroke(100));
	    setViewPosition(new Rectangle((int)view_position.x, (int)view_position.y, (int)view_position.width, (int)view_position.height));
	    g.draw(view_position);
	 }
	 else {
	    g.setColor(OVERVIEW_NONSTYLIZED_VIEW_BORDER_COLOR);
	    setViewPosition(new Rectangle((int)view_position.x, (int)view_position.y, (int)view_position.width, (int)view_position.height));
	    g.draw(view_position);
	 }
       }
...
private void drawFakeWorkingSet (Graphics2D g)
{
   ...
   Paint p = new GradientPaint((int)view_position.x, loc.y, OVERVIEW_FAKE_TOP_COLOR, (int)(view_position.x + view_position.width), loc.y + sz.height, OVERVIEW_FAKE_BOTTOM_COLOR);
   g.setPaint(p);
