...
void makeRoomFor(BudaBubble bb)
{
   ...
   if (best != null) {
      Collection<BudaBubble> del = new ArrayList<BudaBubble>();
...
private Collection<Adjustment> generateMoveSets(Configuration cur,Adjustment mvd)
{
   ...
   if (mvd.hasOverlap()) return new ArrayList<Adjustment>();

   Map<BubbleProxy,List<Movement>> overlapped = new HashMap<BubbleProxy,List<Movement>>();
   ...
   Map<BubbleProxy,List<BubbleProxy>> possibles = new HashMap<BubbleProxy,List<BubbleProxy>>();
   Map<BubbleProxy,List<Movement>> cascaded = new HashMap<BubbleProxy,List<Movement>>();
...
{
   List<BubbleProxy> rslt = new ArrayList<BubbleProxy>();
...
private static class BubbleProxy {
   ...
   double getMovementDistance() {
      return Point.distance(start_place.x,start_place.y,curr_place.x,curr_place.y);
    }
