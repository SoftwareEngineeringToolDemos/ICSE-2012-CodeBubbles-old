...
BudaTaskShelf(BudaTopBar topbar, BudaTask [] tasks, Point popuppoint)
{
   ...
   tree_view.setBackground(Color.WHITE);
...
/********************************************************************************/
/*										*/
/*	Image showing methods (Still work in progress)				*/
/*										*/
/********************************************************************************/

/*********************
private class Hoverer extends BudaHover {
   private PopupFactory popup_factory;
   private Popup my_popup;

   protected Hoverer()
   {
      super(task_tree);
      popup_factory = new PopupFactory();
   }

   @Override public void endHover()
   {
      if (my_popup != null) my_popup.hide();

   }

   @Override public void handleHover(MouseEvent e)
   {
      TreePath tp = task_tree.getPathForLocation(e.getX(), e.getY());
      DefaultMutableTreeNode dmtn = (DefaultMutableTreeNode) tp.getLastPathComponent();
      if (!(dmtn.getUserObject() instanceof BudaTask)) return;
      BudaTask tsk = (BudaTask) dmtn.getUserObject();
      my_popup = popup_factory.getPopup(BudaTaskShelf.this, new PopupPanel(tsk.getImage()), e.getX()+popup_point.x, e.getY()+popup_point.y);
      my_popup.show();
   }

}// end of class Hoverer
...
private class PopupPanel extends JPanel {
   ...
   @Override public void paintComponent(Graphics g)
   {
      ...
      g2.drawImage(my_image, null, this);
   }
}
***************/
