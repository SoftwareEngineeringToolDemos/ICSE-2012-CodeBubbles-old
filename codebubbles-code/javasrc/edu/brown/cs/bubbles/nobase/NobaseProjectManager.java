...
class NobaseProjectManager implements NobaseConstants, NobaseAst
{
...
NobasePathSpec createPathSpec(File src,boolean user,boolean exclude) {
   return new NobasePathSpec(src,user,exclude);
}
...
private void outputTreeNames(NobaseProject pp,NobaseFile file,ISemanticData sd,IvyXmlWriter xw)
{
   ...
   NobaseAst.NobaseAstNode root = sd.getRootNode();
...
private static class NameWalker extends NobaseAstVisitor {
   ...
   @Override public boolean visit(Declaration d) {
      if (d.getInitializer() != null && 
            d.getInitializer() instanceof FunctionConstructor) {
         return true;
       }
      return false;
    }
   
   @Override public boolean visit(MultiDeclaration d) {
      return true;
    }
   
   @Override public boolean visit(FunctionConstructor fc) {
      return true;
    }
   
   @Override public boolean visitStatement(Statement s) {
      return false;
    }
...
{
   NobaseProject p = new NobaseProject(nobase_main,nm,pf);
