...
public File getWorkingDirectory()
{
   ...
   try {
      String pnm = config_attrs.get(NobaseConfigAttribute.PROJECT);
      if (pnm == null) return null;
      NobaseProject pp = NobaseMain.getNobaseMain().getProject(pnm);
      if (pp == null) return null;
      String files = config_attrs.get(NobaseConfigAttribute.MODULE);
...
public List<String> getCommandLine(NobaseDebugManager dbg,int port) throws NobaseException
{
   ...
   cmdargs.add("--debug-brk=" + port);
  // cmdargs.add("--remote_debugger");
   addVmArgs(cmdargs);

   String file = config_attrs.get(NobaseConfigAttribute.MODULE);
