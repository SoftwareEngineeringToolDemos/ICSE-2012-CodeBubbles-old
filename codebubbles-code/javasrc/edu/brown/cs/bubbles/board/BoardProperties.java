...
public Color getColor(String prop,Color dflt)
{
   ...
   Color c = BoardAttributes.getColorByName(s);
   if (c == null) c = dflt;
...
public Color getColorOption(String prop,Color dflt)
{
   ...
   if (s != null && s.length() > 0) {
      try {
	 int[] rgba = new int[4];
	 rgba[3] = Integer.parseInt(s.substring(0, 2), 16);
	 rgba[0] = Integer.parseInt(s.substring(2, 4), 16);
	 rgba[1] = Integer.parseInt(s.substring(4, 6), 16);
	 rgba[2] = Integer.parseInt(s.substring(6, 8), 16);
