		...
		MouseListener
{
   ...
   @Override protected void paintComponent(Graphics g) {
	 ...
	 if (BDDT_THREADS_TOP_COLOR.getRGB() != BDDT_THREADS_BOTTOM_COLOR.getRGB()) {
	    Paint p = new GradientPaint(0f,0f,BDDT_THREADS_TOP_COLOR,
					   0f,sz.height,BDDT_THREADS_BOTTOM_COLOR);
	    g2.setPaint(p);
	  }
	 else {
	    g2.setColor(BDDT_THREADS_TOP_COLOR);
	  }
	 g2.fill(r);
...
private class TransferBubble implements Transferable, BudaConstants.BudaDragBubble {
   ...
   TransferBubble(int [] indices) {
      thread_entries = new ArrayList<BumpThread>();
