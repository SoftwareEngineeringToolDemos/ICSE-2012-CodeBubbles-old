...
private static final int	MAX_PROBLEM = 512;
...
static void outputStackFrame(IJavaStackFrame jsf,int lvl,int vdepth,IvyXmlWriter xw)
	throws DebugException
{
   ...
   try {
	 ...
	 outputValue(ivj.getValue(),ivj,null,vdepth,xw);
       }
    }
...
static void outputValue(IValue val,IJavaVariable var,String name,int lvls,IvyXmlWriter xw)
{
   try {
...
static void outputValue(IEvaluationResult rslt,IvyXmlWriter xw)
{
   xw.begin("EVAL");
   ...
   if (val != null) {
      outputValue(val,null,rslt.getSnippet(),2,xw);
    }
