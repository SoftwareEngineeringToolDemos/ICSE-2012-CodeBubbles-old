...
BumpBreakSet(BumpClient bc)
{
   ...
   break_handlers = new HashMap<BumpBreakpointHandler,File>();
...
@Override public void addLineBreakpoint(String proj,File file,String cls,int line,
					   BumpBreakMode mode)
{
   if (mode == BumpBreakMode.DEFAULT) mode = break_mode;

   bump_client.addLineBreakpoint(proj,file,cls,line,mode.isSuspendVm(),mode.isTrace());
}
...
@Override public BumpBreakpoint findBreakpoint(Element xml)
{
   ...
   return current_breakpoints.get(id);
}
